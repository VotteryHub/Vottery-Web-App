...
import CrossSystemIntegrationPanel from './components/CrossSystemIntegrationPanel';
import { alertRulesEngineService } from '../../services/alertRulesEngineService';
import { alertService } from '../../services/alertService';
import { analytics } from '../../hooks/useGoogleAnalytics';

const CustomAlertRulesEngine = () => {
  ...
  useEffect(() => {
    loadRulesData();
    analytics?.trackEvent('alert_rules_engine_viewed', {
  ...
  const loadRulesData = async () => {
    try {
      ...
      setRuleTemplates(alertRulesEngineService?.getRuleTemplates());
    } catch (error) {
  ...
  const handleCreateRule = async (ruleConfig) => {
    const { data, error } = await alertRulesEngineService?.createCustomRule(ruleConfig);
    if (!error) {
      await loadRulesData();
  ...
  const tabs = [
    ...
    { id: 'integration', label: 'Cross-System', icon: 'Share2' }
  ];

  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Custom Alert Rules Engine | Alert Management</title>
      </Helmet>
      <HeaderNavigation />
      ...
      <main className="container mx-auto px-4 py-6 mt-14">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-muted-foreground">
                Build flexible multi-condition alert system with boolean logic, threshold combinations, and cross-system triggers
              </p>
            </div>
            <Button
              variant="primary"
        ...
        {loading ? (
          ...
          <div>
            {activeTab === 'builder' && (
              <RuleBuilderPanel onCreateRule={handleCreateRule} onRefresh={loadRulesData} />
            )}
            {activeTab === 'active' && (
            ...
            {activeTab === 'templates' && (
              <RuleTemplatesPanel templates={ruleTemplates} onUseTemplate={handleCreateRule} />
            )}
            {activeTab === 'performance' && (
              <RulePerformancePanel rules={alertRules} />
            )}
            {activeTab === 'integration' && (
              <CrossSystemIntegrationPanel />
            )}
          </div>
        )}
      </main>
...
export default CustomAlertRulesEngine;