-- =====================================================
-- VOTTERY ADS STUDIO – UNIFIED SCHEMA
...
-- Ad events + quality metrics; VCG auction; AdSense fallback unchanged
-- =====================================================
...
-- ─── 3. GEO TARGETING (sub-national: states/provinces/regions/etc.) ────────
CREATE TABLE IF NOT EXISTS public.vottery_ad_targeting_geo (
  ...
  region_code TEXT, -- state, province, territory, district, canton, oblast, krai, emirate
  region_name TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.vottery_ads (
  ...
  -- Creative (all types)
  creative JSONB DEFAULT '{}', -- image_url, video_url, headline, body, cta, thumbnail
  -- Participatory: link to election
...
-- ─── 5. SPARK AD REFERENCES (dual-ID tracking: ad ↔ post) ─────────────────────
...
-- ─── 7. AD QUALITY METRICS (aggregated for auction) ────────────────────────
CREATE TABLE IF NOT EXISTS public.ad_quality_metrics (
  ...
  hook_rate NUMERIC(5,2) DEFAULT 0,   -- VIEW_2S / IMPRESSION * 100
  hold_rate NUMERIC(5,2) DEFAULT 0,  -- VIEW_6S / VIEW_2S * 100
  neg_score NUMERIC(10,2) DEFAULT 0,  -- weighted HIDE + REPORT
  quality_score NUMERIC(8,2) DEFAULT 100,
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- ─── 8. PLACEMENT SLOTS (TikTok vs Facebook style; web-only right column) ───
...
-- ─── 9. ADMIN CONFIG (min budgets; adjustable from admin panel) ──────────────
...
INSERT INTO public.vottery_ads_admin_config (key, value_json) VALUES
  ('min_daily_budget_cents', '500'),       -- $5
  ('min_campaign_budget_cents', '10000'),  -- $100
...
-- Ad groups: via campaign ownership
...
-- Spark references: via ad ownership
...
-- Ad events: user can insert own; system/service can insert any (use service role for backend)
...
-- Advertisers need to read events for their ads (analytics)
...
-- Quality metrics: read by advertiser and by auction (anon/authenticated for serving)
...
-- Admin config: public read (min budgets etc.)
...
CREATE TRIGGER trg_ad_events_emergency_safety
  ...
  EXECUTE FUNCTION public.check_ad_safety_on_report();

-- ─── 12. BACKWARD COMPAT: sponsored_elections view (optional) ─────────────
-- Servers that still read sponsored_elections can use a view over vottery_ads (participatory only)
-- or keep using existing sponsored_elections table; new studio writes to vottery_ads.
-- No drop of sponsored_elections; coexistence.
